          ...
          <div className="user-name-userid">
            <img src={profileImg ? (profileImg) : Profile} alt="" />
              <div className='L-user'>
                <h1>{modelDetails ? (modelDetails.ModelName) : "Pulkit"}</h1>
                <span>{modelDetails ? (modelDetails.ModelUserName) : "@Pulkit7"}</span>
            </div>
          </div>
        </Link>
        <MoreHorizIcon onClick={()=>setLogOutExit(!logOutExit)} className='vert'/>
          ...
          {logOutExit && (
            <div className="logOutExitContainer">
              ...
              <Link to="/" style={{width:"100%"}}><button>Log out @Pulkit7</button></Link>
            </div>
          )}
      </div>
